...
- **Granular still renders silence** (the last of the original 6 silent
  engines unaddressed; Additive, PhaseShape, NoiseChaos, Resonator, and
  FM/PM's dedicated engine have each shipped as separate, independently-
  verified (but still-open, as of writing) PRs -- Phase 4/10). The UI is
  already honest about this remaining gap (dashed node rings, disabled
  pills, the note line in `OperatorEditorPanel`), which is the right call
  for now -- but it means 1 of the 8 headline "8-engine algorithmic
  synthesizer" engines still doesn't exist yet on `main`. If "8-engine" is
  the product's core pitch, closing this out (and merging the 5 open engine
  PRs) remains the critical path, ahead of more UI/FX work.
