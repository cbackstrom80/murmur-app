...
- **5 of 8 operator engines still render silence** (Additive, Granular,
  NoiseChaos, Resonator, and FM/PM's dedicated engine -- Phase 4/10). PhaseShape
  (Engine Type 5) shipped this pass (CZ-style phase distortion + wavefold, see
  DSP_ENGINE.md) -- one of several previously-silent engines being landed as
  separate, independently-verified PRs (see FM/PM's and NoiseChaos's own PRs).
  The UI is already honest about the remaining 5 (dashed node rings, disabled
  pills, the note line in `OperatorEditorPanel`), which is the right call for
  now -- but it means 5 of the 8 headline "8-engine algorithmic synthesizer"
  engines still don't exist yet. If "8-engine" is the product's core pitch,
  this is the actual critical path, ahead of more UI/FX work.
