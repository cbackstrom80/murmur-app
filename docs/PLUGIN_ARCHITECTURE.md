...
- **618 host-automatable parameters** (`juce::AudioProcessorValueTreeState`) --
...
32-sample sub-blocks -- this is the suite that actually drives all 618
...
(8 -> 270 -> 361 -> 501 -> 578 -> 618) -- this is not a stale result from an earlier, smaller set.
...
**IMPLEMENTED, 618 parameters.** The original design here exposed only the 8
...
Engine Type 8 (Resonator) shipped (`resonatorStructure`/`resonatorDecay`/
`resonatorDamping`/`resonatorBrightness`/`resonatorModeCount`, +5 fields per
operator, +40 total) -- one of the 6 previously-silent operator engines
gaining real automatable parameters.
...
| 8 operators | 112 | engine, waveform, morph, pulseWidth, wavetableFramePosition, frequencyRatio, fixedFrequencyHz, keyTrack, level, resonatorStructure, resonatorDecay, resonatorDamping, resonatorBrightness, resonatorModeCount (x8) |
...
3. **A saved session must round-trip every automatable field's current
   ...
   rather than a second, divergent store of the same ~618 values.

`auval` reports exactly 618 published parameters (confirmed:
...
`AudioParameterChoice`, for implementation uniformity across all ~618
...
4. The real PLAY/DESIGN/LAB UI (Phase 17) -- `GenericAudioProcessorEditor` is a
   placeholder (now showing all 618 real parameters as generic sliders, not an
   empty list, but still not a step toward the signature UI -- 618 flat
