...
Engine Type 6 (Granular) shipped (`grainDensity`/`grainSizeMs`/
`grainPositionJitter`/`grainPitchJitter`, +4 fields per operator, +32 total)
-- the last of the 6 previously-silent operator engines to gain real
...
| 8 operators | 104 | engine, waveform, morph, pulseWidth, wavetableFramePosition, frequencyRatio, fixedFrequencyHz, keyTrack, level, grainDensity, grainSizeMs, grainPositionJitter, grainPitchJitter (x8) |
