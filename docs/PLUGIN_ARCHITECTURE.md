...
- **578 host-automatable parameters** (`juce::AudioProcessorValueTreeState`) --
...
32-sample sub-blocks -- this is the suite that actually drives all 578
...
(8 -> 270 -> 361 -> 501 -> 578) -- this is not a stale result from an earlier, smaller set.
...
**IMPLEMENTED, 578 parameters.** The original design here exposed only the 8
...
slot) for its multiband/diffuser/early-late architecture.
...
3. **A saved session must round-trip every automatable field's current
   ...
   rather than a second, divergent store of the same ~578 values.

`auval` reports exactly 578 published parameters (confirmed:
...
4. The real PLAY/DESIGN/LAB UI (Phase 17) -- `GenericAudioProcessorEditor` is a
   placeholder (now showing all 578 real parameters as generic sliders, not an
   empty list, but still not a step toward the signature UI -- 578 flat
