...
- **594 host-automatable parameters** (`juce::AudioProcessorValueTreeState`) --
...
32-sample sub-blocks -- this is the suite that actually drives all 594
...
(8 -> 270 -> 361 -> 501 -> 578 -> 594) -- this is not a stale result from an earlier, smaller set.
...
**IMPLEMENTED, 594 parameters.** The original design here exposed only the 8
...
Engine Type 7 (NoiseChaos) shipped (`noiseVariant`/`noiseRate`, +2 fields per
operator, +16 total) -- the first of the 6 previously-silent operator engines
to gain real automatable parameters.
...
| 8 operators | 88 | engine, waveform, morph, pulseWidth, wavetableFramePosition, frequencyRatio, fixedFrequencyHz, keyTrack, level, noiseVariant, noiseRate (x8) |
...
3. **A saved session must round-trip every automatable field's current
   ...
   rather than a second, divergent store of the same ~594 values.

`auval` reports exactly 594 published parameters (confirmed:
...
`AudioParameterChoice`, for implementation uniformity across all ~594
...
4. The real PLAY/DESIGN/LAB UI (Phase 17) -- `GenericAudioProcessorEditor` is a
   placeholder (now showing all 594 real parameters as generic sliders, not an
   empty list, but still not a step toward the signature UI -- 594 flat
