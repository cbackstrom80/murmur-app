...
Engine Type 5 (PhaseShape) shipped (`phaseBend`/`phaseFold`/`phaseAsymmetry`/
`phaseShape`, +4 fields per operator, +32 total) -- one of the 6
previously-silent operator engines gaining real automatable parameters.
...
| 8 operators | 104 | engine, waveform, morph, pulseWidth, wavetableFramePosition, frequencyRatio, fixedFrequencyHz, keyTrack, level, phaseBend, phaseFold, phaseAsymmetry, phaseShape (x8) |
