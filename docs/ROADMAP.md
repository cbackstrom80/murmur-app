...
| 10 | Additional engines (additive, phase/shape, noise, resonator, granular) | **PARTIAL** -- Granular (Engine Type 6) **IMPLEMENTED** this pass: fixed-capacity grain pool reading the same wavetableId-loaded data the Wavetable engine uses, see DSP_ENGINE.md (one of the 6 previously-silent engines, each landed as a separate, independently-verified PR -- see FM/PM's, NoiseChaos's, PhaseShape's, Additive's, and Resonator's own PRs). This closes out Phase 10's full engine set, pending merge |
