...
| 10 | Additional engines (additive, phase/shape, noise, resonator, granular) | **PARTIAL** -- Resonator (Engine Type 8) **IMPLEMENTED** this pass: noise-excited modal filter bank via a new `Biquad::setBandpass()` primitive, see DSP_ENGINE.md (one of several previously-silent engines being landed as separate, independently-verified PRs -- see FM/PM's, NoiseChaos's, PhaseShape's, and Additive's own PRs). Granular still renders silence until implemented |
