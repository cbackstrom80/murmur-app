...
| 10 | Additional engines (additive, phase/shape, noise, resonator, granular) | **PARTIAL** -- NoiseChaos (Engine Type 7) **IMPLEMENTED**: 7 variants (white/pink/brown/blue/S&H/smooth-random/dust), seeded via `dsp::DeterministicRng`, see DSP_ENGINE.md. Being built out one engine/PR at a time otherwise, in order of how much existing infrastructure each reuses; additive/phase-shape/resonator/granular still render silence until their turn. (FM/PM, also once part of this phase, now ships under Phase 4 above.) |
