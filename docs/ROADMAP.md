...
| 10 | Additional engines (additive, phase/shape, noise, resonator, granular) | **PARTIAL** -- NoiseChaos (Engine Type 7) and PhaseShape (Engine Type 5) **IMPLEMENTED**: 7 noise variants (white/pink/brown/blue/S&H/smooth-random/dust) seeded via `dsp::DeterministicRng`, and CZ-style phase distortion + wavefold, respectively -- see DSP_ENGINE.md. Being built out one engine/PR at a time otherwise, in order of how much existing infrastructure each reuses; additive/resonator/granular still render silence until their turn. (FM/PM, also once part of this phase, now ships under Phase 4 above.) |
