...
| 10 | Additional engines (additive, phase/shape, noise, resonator, granular) | **PARTIAL** -- NoiseChaos (Engine Type 7), PhaseShape (Engine Type 5), Additive (Engine Type 4), and Resonator (Engine Type 8) **IMPLEMENTED**: 7 noise variants seeded via `dsp::DeterministicRng`, CZ-style phase distortion + wavefold, a 64-partial coupled-form oscillator bank with tilt/odd-even/stretch, and a noise-excited modal filter bank via a new `Biquad::setBandpass()` primitive, respectively -- see DSP_ENGINE.md. Granular still renders silence until its turn. (FM/PM, also once part of this phase, now ships under Phase 4 above.) |
