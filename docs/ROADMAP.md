...
| 10 | Additional engines (additive, phase/shape, noise, resonator, granular) | **PARTIAL** -- NoiseChaos (Engine Type 7), PhaseShape (Engine Type 5), and Additive (Engine Type 4) **IMPLEMENTED**: 7 noise variants seeded via `dsp::DeterministicRng`, CZ-style phase distortion + wavefold, and a 64-partial coupled-form oscillator bank with tilt/odd-even/stretch, respectively -- see DSP_ENGINE.md. Being built out one engine/PR at a time otherwise, in order of how much existing infrastructure each reuses; resonator/granular still render silence until their turn. (FM/PM, also once part of this phase, now ships under Phase 4 above.) |
