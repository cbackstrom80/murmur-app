...
| 10 | Additional engines (additive, phase/shape, noise, resonator, granular) | **PARTIAL** -- Additive (Engine Type 4) **IMPLEMENTED** this pass: 64-partial coupled-form oscillator bank with tilt/odd-even/stretch, see DSP_ENGINE.md (one of several previously-silent engines being landed as separate, independently-verified PRs -- see FM/PM's, NoiseChaos's, and PhaseShape's own PRs). Granular and Resonator still render silence until implemented |
