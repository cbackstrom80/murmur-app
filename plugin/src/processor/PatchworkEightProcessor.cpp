...
namespace pw8::plugin
{
    ...
    void PatchworkEightProcessor::pushLiveParametersToEngine(render::Engine& engine) noexcept
    {
        ...
        for (std::size_t op = 0; op < kNumOperators; ++op)
        {
            ...
            params.phaseBend = loadF(ptrs[9]);
            params.phaseFold = loadF(ptrs[10]);
            params.phaseAsymmetry = loadF(ptrs[11]);
            params.phaseShape = loadF(ptrs[12]);
    ...
    void PatchworkEightProcessor::syncPatchFromAllParameters()
    {
        ...
        for (std::size_t op = 0; op < kNumOperators; ++op)
        {
            ...
            o.phaseBend = loadF(ptrs[9]);
            o.phaseFold = loadF(ptrs[10]);
            o.phaseAsymmetry = loadF(ptrs[11]);
            o.phaseShape = loadF(ptrs[12]);
        }
